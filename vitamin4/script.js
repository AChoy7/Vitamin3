...
const question2 = () => {
  ...
  addTodoButton.addEventListener("click", (event) =>{
    const inputText = taskName.value.trim();
...
const question3 = () => {
  ...
  const updateMessage = () => {
    /** YOUR CODE HERE */
    const firstText = firstNameInput.value.trim();
    const lastText = lastNameInput.value.trim();

    if (firstText || lastText) {
      message.textContent = `Hello ${firstText} ${lastText}!`;
    }
    else{
